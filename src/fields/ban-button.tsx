...
const BanButton: React.FC<FieldProps> = (props) => {
    ...
    if (record.banned) {
        return (
            < Button startIcon={< UndoIcon />} label="Unban" variant="text" onClick={handleClick} disabled={loading} />
        );
    }
    else {
        return (
            < Button startIcon={< GavelIcon />} label="Ban" variant="text" onClick={handleClick} disabled={loading} />
        );
    }
}
